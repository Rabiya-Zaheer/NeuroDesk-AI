...
const primaryLinks = [{ label: "Home", href: "/dashboard" }];
...
export function DashboardSidebar({ workspaces }: { workspaces: WorkspaceSummary[] }) {
    ...
    <aside className="flex h-screen w-64 shrink-0 flex-col border-r border-(--color-border) bg-(--color-surface) px-4 py-5">
      ...
      <nav className="flex flex-col gap-1">
        {primaryLinks.map((link) => (
          <SidebarLink key={link.href} label={link.label} href={link.href} icon={Home} active={pathname === link.href} />
        ))}
      </nav>
      ...
      <nav className="mt-2 flex flex-1 flex-col gap-1 overflow-y-auto">
        {workspaces.length === 0 && (
          <p className="px-2.5 py-2 text-xs text-(--color-ink-faint)">No workspaces yet</p>
        )}
        {workspaces.map((ws) => {
